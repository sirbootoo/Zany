...
async function init() {
  try {
    ...
    if (user) {
      ...
      collectionAddress = await getVariableValues("collectionAddress", null);
      entryFee = await getVariableValues("entryFee");      
      numberOfGamesCreated = await getVariableValues("numberOfGamesCreated");
      gamesListTableData();
      User = user;
      walletAddress = user.get("ethAddress");
      listenToEvents("NumberDrawn", (event) => {
        document.getElementById("gameResult").innerHTML = event.returnValues._gameID + " - " + event.returnValues.winningNumber;
...
function listenToEvents(name, callback) {
  return Contract.events[name]({}).on('data', callback)
}
